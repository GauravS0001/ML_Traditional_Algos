...
class GradientBoosting:
    ...
    def fit(self, X, y):
        self.models = []
        self.initial_prediction = [sum(y) / len(y)] * len(y)
        residuals = [y_i - self.initial_prediction[i] for i, y_i in enumerate(y)]
        ...
        for _ in range(self.n_estimators):
            ...
            residuals = [residuals[i] - self.learning_rate * predictions[i] for i in range(len(residuals))]

    def predict(self, X):
        predictions = [self.initial_prediction[i] for i in range(len(X))]
...
gb_model = GradientBoosting(n_estimators=5, learning_rate=0.1, max_depth=2)
